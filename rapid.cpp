...
void rapid_band_affine(string file1, string file2)
{
    ...
    for (int i = 1; i <= m; i++) {
        curr_loc = curr_loc + col;
        write_dram(add_dram_val(curr_loc-2,sigmae), curr_loc);
        for (int j = max(1,i-b/2); j <= min(n,i+b/2); j++) {
            curr_loc_a = curr_loc + 2*j;
            curr_loc_c = col*(i-1) + (j-1);
            curr_loc_b = curr_loc_a + 1;
            curr_loc_c2 = curr_loc_c - 2;
            write_dram_loc(curr_loc_a-2,curr_loc_b);
            add_write_dram(curr_loc_a-col-2,curr_loc_c2, curr_loc_c);
        }
        for (int j = max(1,i-b/2); j <= min(n,i+b/2); j++) {
            curr_loc_a = curr_loc+2*j;
            ...
            curr_loc_c2 = curr_loc_c - 2;
            curr_loc_b = a+1;
            sigma1 = sigma_cal(curr_loc_b);
            sigma2 = sigma_cal(curr_loc_c);
            tmp,pos = find_min_tmp_aff(add(curr_loc_a,curr_loc_c),
                    add_dram_val(curr_loc_b,sigma1),
                    add_dram_val(curr_loc_c2,sigma2));
        }
    }
...
void rapid_band_lin(string file1, string file2)
{
    ...
    for (int i = 1; i <= m; i++) {
        ...
        for(j=1;j<=n;j++)
        {
            write_dram(add_dram_val(curr_loc-2,sigma), curr_loc);
        }
        for (int j = max(1,i-b/2); j <= min(n,i+b/2); j++) {
            curr_loc_a = curr_loc+2*j;
            curr_loc_c = col*(i-1) + (j-1);
        ...
        for (int j = max(1,i-b/2); j <= min(n,i+b/2); j++) {
            ...
            curr_loc_c2 = curr_loc_c - 2;
            curr_loc_b = a+1;
            find_min_tmp_lin(add(curr_loc_a,curr_loc_c),
                    add_dram_val(curr_loc_b,sigma),
                    add_dram_val(curr_loc_c2,sigma));
        }
    }
    string res = find_min_path(max_loc + m + b);
...
void rapid(string file1, string file2)
{
    ...
    for (int i = 1; i <= m; i++) {
        curr_loc = curr_loc + 2*col;
        for(j=1;j<=n;j++)
        {
            write_dram(add_dram_val(curr_loc-col,sigma), curr_loc);
        }
        for (int j = 1; j <= n; j++) {
            curr_loc_a = curr_loc+2*j;
            curr_loc_c = col*(i-1) + (j-1);
        ...
        for (int j = 1; j <= n; ++j) {
            ...
            curr_loc_c2 = curr_loc_c - 2;
            curr_loc_b = a+1;
            find_min_tmp(add(curr_loc_a,curr_loc_c),
                    add_dram_val(curr_loc_b,sigma), 
                    add_dram_val(curr_loc_c2,sigma));
        }
    }
